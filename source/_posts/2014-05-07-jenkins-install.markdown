...
####Q: ssh 连接到slave时候,输入正确的账号密码,但经常遇到Permission denied 问题
####A: 一种是由于authorized_keys 权限设置不对导致,设置authorized_keys 权限为644,一种由于账号没配置对导致,可以执行以下命令  
`mkpasswd.exe -l > /etc/pass`   
